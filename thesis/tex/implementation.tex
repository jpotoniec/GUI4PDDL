...
Zadaniem oprogramowania wyznaczającego plan (tzw. \JP{PPR planera}planera) jest ustalanie planu, czyli sekwencji akcji, \JP{PPR wyznaczanie planu, czyli, to wykreślamy: przetwarzanie problemów automatycznego planowania, uzyskując na wyjściu plan, czyli} prowadzącej do osiągnięcia stanu końcowego ze stanu początkowego problemu. Opis problemu wyrażony jest za pomocą języka automatycznego planowania (np. STRIPS, PDDL). 

W przypadku języka PDDL, wykorzystywanego w niniejszej pracy, instancja problemu\JP{PPR instancja problemu, zadanie funkcjonuje jako odpowiednik słowa problem w rozumieniu teorii złożoności obliczeniowej} automatycznego planowania składa się z dwóch plików: pliku dziedziny, w którym opisana jest dziedzina zadania oraz pliku problemu, opisującego stan początkowy oraz docelowy. Współpraca narzędzia GUI4PDDL z planerami wymaga więc możliwości\JP{PPR możliwości} uzyskania planu na podstawie aktualnej zawartości dwóch, wyżej wymienionych plików. Ponadto należy zagwarantować możliwość zmiany algorytmu planowania lub innych opcji, które są dostępne dla poszczególnych narzędzi. Gotowy plan powinien być również przedstawiony użytkownikowi w postaci czytelnej.

Obecnie dostępnych jest wiele planerów, korzystających z języka PDDL, różniących się dostępnymi algorytmami, sposobem uruchamiania, liczbą\JP{PPR argumenty są policzalne, więc: liczbą} oraz rodzajem przyjmowanych na wejściu argumentów, a także strukturą strumienia wyjściowego. Ze względu na to, że programy wyznaczające plan tworzone są często z przeznaczeniem na konkurs IPC (\emph{International Planning Competition}), dotychczas nie zdefiniowano standardu uruchamiania tego typu narzędzi, który obejmowałby m.in. ujednolicenie argumentów linii poleceń oraz dodatkowych, wykorzystywanych komend powłoki, często specyficznych dla systemu operacyjnego.\JP{PPR serio różnią się formatem? myślałem, że IPC ma standardowy format wyjściowy, bo inaczej nie byłoby jak weryfikować planów?}.

Analiza przykładowych narzędzi (\emph{FastDownward}\footnote{http://www.fast-downward.org/} , \emph{SATPLAN}\footnote{http://www.cs.rochester.edu/users/faculty/kautz/satplan/}, \emph{FastForward}\footnote{http://fai.cs.uni-saarland.de/hoffmann/ff.html}) pod kątem inicjalizacji procesu planowania pozwoliła zidentyfikować\JP{PPR pozwoliła zidentyfikować} cechy wspólne oraz różnice pomiędzy planerami, opisane w tabeli \ref{plannersTable}.\JP{PPR opisane w tabeli \ref{plannersTable}.}
\begin{table}[h]
\centering
...
Analogiczny skrypt uruchomieniowy można wykonać również dla innego oprogramowania wyznaczającego plan. Kod takiego skryptu dla planera \emph{SATPLAN} przedstawiony jest w przykładzie~\ref{lst:satplan}. \JP{PPR Przykładowy skrypt dla planera SATPLAN znajduje się \ldots.}
  
\JP{PPR Ten przykład trzeba uprościć, zawiera za dużo rzeczy niezwiązanych z tematem. Usuńcie die i usage na rzecz wstawienia odpowiedniego kodu w miejscu ich wywołania, tak samo usuńcie echo przed i po wywołaniu SATPLAN}
\begin{Code}
...
\end{figure}
Standardowo, dostępny jest wstępnie skonfigurowany planer \emph{FastDownward}. Okno preferencji, przedstawione na rysunku~\ref{fig:preferences_window} \JP{PPR przedstawione na rysunku \ref{fig:preferences_window}} udostępnia następujące opcje:
\JP{PPR W tym wypunkowaniu coś trzeba zrobić wizualnie. Nazwy opcji powinny chyba albo się wyróżniać, albo być sensownie zintegrowane z tekstem. Może np przejść na describe i tylko w nawiasach pisać, że na rysunku jest to oznaczone nr jakimśtam?}
...
Nowy sposób zakończenia procesu opiera się na wykorzystaniu natywnych metod wspieranych systemów operacyjnych (Windows oraz Linux \JP{a co z Makami? Bez tego dr AŁ będzie nieszczęśliwa}) poprzez bibliotekę \textit{JNA} (\textit{Java Native Access})\footnote{https://github.com/twall/jna}. Biblioteka ta pozwala na wykonywanie operacji specyficznych dla danej platformy bez konieczności dodatkowej konfiguracji. Za jej pomocą pobierany jest numer PID skryptu, który następnie podawany jest jako argument specyficznych dla systemu poleceń powłoki, odpowiedzialnych za przerywanie procesów. W przypadku systemu Windows jest to polecenie \emph{taskkill}, w przypadku rodziny Linux -- \emph{kill}, z odpowiednio przetworzonym\JP{PPR jakoś to po polsku proszę napisać, a nie robić kalkę z angielskiego} drzewem podprocesów otrzymanym z komendy \emph{pstree}.
