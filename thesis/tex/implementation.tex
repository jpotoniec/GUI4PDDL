...
Jak wspomniano, planner \textit{FastDownward} w wersji dla systemów z rodziny \textit{Linux} posiada odpowiedni plik uruchomieniowy w katalogu \path{src} instalacji. W przypadku systemu \textit{Windows} należy stworzyć osobny program wsadowy (ang \textit{batch file}).

\begin{lstlisting}[frame=single,caption={Przykładowy skrypt uruchomieniowy (dla systemu Windows) FastDownward z obsługą prostych błędów (skrypt ten znajduje się w katalogu resources projektu pl.poznan.put.cs.gui4pddl).}]  % Start your code-block
@ECHO off
SET basedir=%~dp0
call :getargc argc %*
...
Analogiczny skrypt uruchomieniowy można wykonać dla innego oprogramowania wyznaczającego plan, np. \textit{SATPLAN}.
