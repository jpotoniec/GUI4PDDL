...
Zalecanym sposobem przetwarzania abstrakcyjnych drzew składniowych jest analizator drzew
(ang \emph{tree parser}). %\JP{Skoro to jest tree parser to dlaczego tego nie tłumaczyć jako analizator drzew? Z opisu na http://www.antlr2.org/doc/sor.html wynika mi, że on właśnie analizuje drzewa, a nie używa drzew do analizy czegoś innego?}.
...
nadrzędnych jest przekazywana ,,w dół''. Wprowadzenie analizatorów drzew 
...
analizatora leksykalnego, składniowego, analizatora drzew lub połączoną specyfikację
...
\begin{Code}[h!]
\begin{lstlisting}[label=antlr_astcode, caption=Gramatyka analizatora drzew przetwarzającego listę wymagań, numbers=left, frame=single]
tree grammar PDDL;
...
analizator drzew. Przetwarza on
