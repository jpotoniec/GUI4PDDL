...
\subsection{Początek\JP{Rys historyczny}}
W 2001 roku pojawił się na rynku mało znany projekt typu opensource\JP{ma być spójnie: na końcu rozdziału 2.3 jest po polsku, więc musicie wybrać jeden wariant} – \emph{Eclipse 1.0}. Program ten został opisany jako ,,zintegrowane środowisko deweloperskie do wszystkiego i~do nieczego w szególności”\JP{źródło cytatu}. W założeniu nie miał to być kolejny zestaw narzędzi, a~w~pełni skalowanle i~rozszerzalne środowisko programistyczne. Razem z programem, udostępniono szereg narzędzi dla deweloperów, umożliwiających rozwijanie projektu. Początkowo Eclipse powstał tylko po to, by dowieść, że można zbudować środowisko rozszerzalne w tak dużym zakresie. Dzięki społeczności, bardzo szybko środowisko to rozwinęło się na wiele języków programowania.\JP{skąd wiedza w tym akapicie? Z pewnością ciężko przyjąć, że to jest coś, co wszyscy informatycy wiedzą}

Typowy stereotyp\JP{stereotyp chyba nie może nie być typowy} deweloperów\JP{programistów/twórców} open source\JP{to juz trzeci wariant tego samego, patrz pierwszy akapit} ukazuje bezinteresownego programistę, nocami pracującego nad poprawą błędów i implementacją własnych pomysłów. W Eclipse, początkowe fragmenty kodu były napisane przez programistów firmy \emph{IBM}. Pracownicy ci dostawali pełne wynagrodzenie za pracę nad otwartym projektem, odpowiadaniem na pytania społeczności, naprawą błędów. Początkowy wkład w rozwój Eclipse miały firmy takie jak Borland, IBM, Merant, QNX Software Systems, Rational Software, RedHat, SuSE, a także TogetherSoft\JP{nazwy firm chyba kursywą}. Firmy te mogły dzięki temu liczyć na wypuszczenie na rynek programów komercyjnych bazujących na tym środowisku. 
...
Na początku Eclipse było znane jako czysta platforma, dziś jest to zaawansowane środowisko programistyczne z ponad 250 aktywnymi projektami\JP{jakimi projektami? tymi w workspace? może lepszym słowem byłoby: rozszerzeniami?}. W Internecie można znaleźć narzędzia i~wtyczki do Eclipse umożliwające pisanie i testowanie kodu niemal w~każdym języku programowania.

\subsection{Platforma}

Program Eclipse zbudowany jest w języku \emph{Java}, wymaga więc zainstalowanej Javy na komputerze użytkownika.  Całe środowisko składa się z \emph{pluginów}\JP{czy koniecznie potrzebujemy ten angielski termin?}, czyli osobnych wtyczek współpracyjących ze sobą, oraz rdzenia samego programu (ang. \emph{Eclipse kernel}). Wtyczka to nic innego, jak plik \emph{JAR} z manifestem, określającym zachowanie i przeznaczenie modułu. W manifestach zawierane są również informacje na temat zależności między wtyczkami:
